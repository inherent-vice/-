...
Desktop GUI utility for the KAP 파생상품평가본부. The user inputs Korean derivative bond/note tickers (e.g., `KR6HN0008746  하나증권(DLB)2681`), and the app locates the matching **termsheet PDF** on DART (금융감독원 전자공시시스템) and saves it to `~/Downloads`. Secondary button verifies issuance status (발행취소 vs 발행됨) via 증권발행실적보고서.
...
3. **PDF body scan (last resort)** — download top 15 candidate PDFs and extract first 5 pages with pdfplumber; search those for the round number.
...
`kap_logo.png` is bundled (read-only). `issuers.json` and `state/` sit next to the exe (user-writable).
...
**Syntax check:**
```
py -c "import py_compile; py_compile.compile('dart_auto.py', doraise=True)"
```

No test suite exists. Manual verification is by running the GUI and inspecting the log pane.
